...
ggplot(popData,aes(t, blibN))+geom_point()
...
ggplot(blibPheno,aes(time,Heterozygosity))+geom_count(alpha = 0.02)+geom_smooth(method="lm")
...
ggplot(blibPheno,aes(x = time))+geom_count(aes(y = (Heterozygosity)), colour = "blue", alpha = 0.01)+geom_smooth(aes(y = Heterozygosity),colour = "red", alpha = 0.01)
hist(blibPheno$exon_intron_ratio)

ggplot(blibPheno%>%select(time, greenAllele1, greenAllele2)%>%gather("key", "value", -time), aes(time,value, colour = key))+geom_count(alpha = 0.05)+geom_smooth()


ggplot(blibPheno%>%select(time, lifeLengthAllele1, lifeLengthAllele2)%>%gather("key", "value", -time), aes(time,value, colour = key))+geom_count(alpha = 0.2)+geom_smooth(colour = "black")

ggplot(blibPheno%>%select(time, turnTorqueAllele1, turnTorqueAllele2)%>%gather("key", "value", -time), aes(time,value, colour = key))+geom_count(alpha = 0.2)+geom_smooth(colour = "black")
...
ggplot(blibPheno%>%select(time, moveAllele1, moveAllele2)%>%gather("key", "value", -time), aes(time,value, colour = key))+geom_count(alpha = 0.4)+geom_smooth(colour = "black")

ggplot(blibPheno%>%select(time, greenAllele1, greenAllele2)%>%gather("key", "value", -time), aes(time,value, colour = key))+geom_smooth(colour = "black")
